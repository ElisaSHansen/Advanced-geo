...
def solve_FDM(n=400):
    ...
    # y'''' ~ (u[i-2] - 4u[i-1] + 6u[i] - 4u[i+1] + u[i+2]) / dx^4
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    # Special near-boundary equations at i = n-2 and i = 1
    # Use the governing equation there with one-sided 5-point formulas
    # --------------------------------------------------------

    # i = 1  -> y'''' using forward stencil:
    # (u0 - 4u1 + 6u2 - 4u3 + u4) / dx^4
    K[2, 0] = 1.0
    K[2, 1] = -4.0
    ...
    # i = n-1 already used by slope BC, so governing equation at i = n-2
    # backward compatible central stencil
    irow = n - 2
    i = n - 2
    K[irow, i-2] = 1.0
    K[irow, i-1] = -4.0
    K[irow, i]   = 6.0 + alpha * dx**4
    K[irow, i+1] = -4.0
    K[irow, i+2] = 1.0
    ...
    # Derivatives / response quantities
    ...
    theta[0] = (-3*y[0] + 4*y[1] - y[2]) / (2.0 * dx)
    theta[-1] = (3*y[-1] - 4*y[-2] + y[-3]) / (2.0 * dx)

    # moment
    M[1:-1] = EI * (y[:-2] - 2.0*y[1:-1] + y[2:]) / dx**2
    M[0] = EI * (2*y[0] - 5*y[1] + 4*y[2] - y[3]) / dx**2
    M[-1] = EI * (2*y[-1] - 5*y[-2] + 4*y[-3] - y[-4]) / dx**2

    V = np.full_like(M, np.nan)

    # Shear from derivative of moment (much more stable)
    V[1:-1] = -(M[2:] - M[:-2]) / (2*dx)
...
def solve_analytical(npts=800):
    ...
    d3y = (
        ex * 2.0 * l**3 * ((-C1 + C2) * cx + (-C1 - C2) * sx)
        + emx * 2.0 * l**3 * ((C3 + C4) * cx + (C3 - C4) * sx)
    )
...
x_fdm, y_fdm, th_fdm, M_fdm, V_fdm = solve_FDM(n=400)
x_an,  y_an,  th_an,  M_an,  V_an  = solve_analytical(npts=800)
...
print("\nHead values")
print("------------------------------")
print(f"Analytical M(0): {M_an[0]/1e6:.3f} MNm   (target {M_head/1e6:.3f} MNm)")
print(f"Analytical V(0): {V_an[0]/1e3:.2f} kN   (target {V_head/1e3:.2f} kN)")
